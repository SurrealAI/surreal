...
from surreal.learner import GAILLearner
...
GAIL_DEFAULT_LEARNER_CONFIG = Config({
    ...
    'algo': {
        ...
        'network': {
            ...
            'clip_discriminator_gradient': True,
            'discriminator_gradient_norm_clip': 10.,
            'actor_regularization': 0.0,
            'critic_regularization': 0.0,
            'discriminator_regularization': 0.0,
            'anneal':{
...
GAIL_DEFAULT_LEARNER_CONFIG.extend(BASE_LEARNER_CONFIG)

GAIL_DEFAULT_ENV_CONFIG = Config({
    'env_name': '',
    'action_repeat': 10,
    'pixel_input': False,
    ...
    'observation': {
        ...
        # 'object': ['cube_pos','gripper_to_cube','cyl_to_hole','angle', 't', 'd', 'hole_pos'],
        'object': ['cube_pos','gripper_to_cube','cyl_to_hole','angle', 't', 'd', 'hole_pos'
    ...
    'demonstration': {
        ...
        'mixing': ['random'], # e.g. ['random', 'uniform']
        'mixing_ratio': [1.0], # e.g. [0.5, 0.5], must sum to one
        'ratio_step': [0.0],
...
GAIL_DEFAULT_ENV_CONFIG.extend(BASE_ENV_CONFIG)

GAIL_DEFAULT_SESSION_CONFIG = Config({
    'folder': '_str_',
    'tensorplex': {
        'update_schedule': {
...
GAIL_DEFAULT_SESSION_CONFIG.extend(LOCAL_SESSION_CONFIG)


class GAILLauncher(SurrealDefaultLauncher):
    def __init__(self):
        learner_class = GAILLearner
        agent_class = PPOAgent
        replay_class = FIFOReplay
        learner_config = GAIL_DEFAULT_LEARNER_CONFIG
        env_config = GAIL_DEFAULT_ENV_CONFIG
        session_config = GAIL_DEFAULT_SESSION_CONFIG
    ...
    def setup(self, argv):
        ...
        parser.add_argument('--demo-path', type=str, default="../../surreal_demos",
                            help='path to load .npy serialized expert demonstrations from')
        ...
        self.env_config.demo_path = args.demo_path
